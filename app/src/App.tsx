...
import type { ViewType, Shop } from '@/types';
...
function App() {
  ...
  // Supabase Auth and Realtime Data
  const { user, userName, role, shopId } = useAuth();
  const shops = useRealtime<Shop>('shops', [], shopId || undefined);
  const activeShop = shops.find(s => s.id === shopId) || shops[0] || { id: 'shop_001', name: 'FRESHCUT X', status: 'open', plan: 'pro', address: '', phone: '', createdAt: new Date() };

  // Fallback info if user metadata is missing
  const displayName = userName || user?.user_metadata?.name || user?.email?.split('@')[0] || 'Admin';
  const displayRole = role || user?.user_metadata?.role || 'Manager';
          ...
          <div className="p-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-white/50">{activeShop.name}</p>
              </div>
            </div>
          </div>
            ...
            <div className="glass-card p-4">
              <div className="flex items-center gap-3">
                <Avatar className="w-10 h-10 ring-2 ring-cyber-neon/30">
                  <AvatarImage src="" />
                  <AvatarFallback className="bg-cyber-gray text-cyber-neon uppercase">
        ...
        <header className="sticky top-0 z-30 bg-cyber-dark/80 backdrop-blur-xl border-b border-white/10">
          <div className="flex items-center justify-between px-6 py-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <Badge
                variant="outline"
                className={`hidden xs:flex ${activeShop.status === 'open'
                  ? 'border-cyber-neon/50 text-cyber-neon bg-cyber-neon/10'
                  : 'border-red-500/50 text-red-400 bg-red-500/10'
                  }`}
              >
                <span className={`w-2 h-2 rounded-full mr-1 sm:mr-2 ${activeShop.status === 'open' ? 'bg-cyber-neon animate-pulse' : 'bg-red-400'
                  }`} />
                <span className="hidden sm:inline">{activeShop.status === 'open' ? 'Ouvert' : 'Fermé'}</span>
                <span className="sm:hidden">{activeShop.status === 'open' ? 'O' : 'F'}</span>
              </Badge>
