...
import type { QueueItem, Client, Barber, Service } from '@/types';
...
export function QueueManagement() {
  ...
  const realtimeQueue = useRealtime<QueueItem>('queueItems', [], shopId || undefined);
                      ...
                      <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
                        <Avatar className="w-10 h-10 sm:w-12 h-12 ring-2 ring-white/10">
                          ...
                          <AvatarFallback className="bg-cyber-gray">
                            {(item.client?.name || item.clientId || '?').charAt(0)}
                          </AvatarFallback>
                        </Avatar>
                        <div className="min-w-0">
                          <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2">
                            <p className="text-white font-medium truncate text-sm sm:text-base">{item.client?.name || item.clientId || 'Client'}</p>
                            {/* Mobile status badge */}
                          ...
                          <div className="flex items-center gap-2 text-[11px] sm:text-sm text-white/50">
                            <span className="truncate">
                              {item.services?.map((s: any) => s.name).join(', ') || 'Service'}
                            </span>
                          </div>
                        </div>
                      </div>
