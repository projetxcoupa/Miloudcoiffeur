...
export function ClientCRM() {
  ...
  const realtimeClients = useRealtime<Client>('clients', [], shopId || undefined);
  ...
  const handleBlockClient = async (clientId: string) => {
    ...
    try {
      const { error } = await supabase
        .from('clients')
        .update({ isBlocked: !client.isBlocked })
        .eq('id', clientId);
  ...
  const handleAddClient = async () => {
    ...
    try {
      const { error } = await supabase.from('clients').insert([{
        ...
        isBlocked: false,
        preferences: [],
        avatar: `https://i.pravatar.cc/150?u=${encodeURIComponent(newClientData.name)}`
      }]);
      if (error) throw error;
          ...
          <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
            ...
            <DialogContent className="bg-cyber-gray border-white/10">
              ...
              <div className="space-y-4 mt-4">
                ...
                />
                <Button className="w-full btn-neon" onClick={handleAddClient}>
                  Créer le client
                </Button>
              </div>
            </DialogContent>
