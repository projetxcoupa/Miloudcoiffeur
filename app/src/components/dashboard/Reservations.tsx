...
export function Reservations() {
  ...
  const realtimeAppointments = useRealtime<Appointment>('appointments', [], shopId || undefined);
  const clients = useRealtime<Client>('clients', [], shopId || undefined);
  const barbers = useRealtime<Barber>('barbers', [], shopId || undefined);
  const servicesData = useRealtime<Service>('services', [], shopId || undefined);
  ...
  const handleAddAppointment = async () => {
    if (!guestName || !guestPhone || !selectedBarber || selectedServices.length === 0 || !selectedTime) {
    ...
    try {
      let clientId;
      const existingClient = clients.find(c => c.phone === guestPhone);
      if (existingClient) {
        clientId = existingClient.id;
      } else {
        // Fallback shopId handling for RLS matching (if user is logged in, use their real shopId)
        const activeShopId = shopId || 'shop_001';
        ...
        // Wait briefly for realtime to catch up, or query the newly created client
        // since .select() triggers an RLS read policy violation on insertions sometimes
        const { data: fetchClient } = await supabase
          .from('clients')
          .select('id')
          .eq('phone', guestPhone)
          .eq('shopId', activeShopId)
          .single();

        clientId = fetchClient?.id;
        if (!clientId) throw new Error("Erreur: ID client introuvable après création.");
      }
      ...
      toast.error('Erreur lors de la création du rendez-vous: ' + e.message);
    }
  };
            ...
            <DialogContent className="bg-cyber-gray border-white/10 max-w-lg max-h-[90vh] overflow-y-auto">
              ...
              <div className="space-y-4 mt-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-sm text-white/60 mb-2 block">Nom du client</label>
                    <Input
                  ...
                  <div>
                    <label className="text-sm text-white/60 mb-2 block">Numéro (Guest)</label>
                    <Input
                      placeholder="Ex: 06..."
                      value={guestPhone}
                ...
                <Button
                  ...
                  disabled={!guestName || !guestPhone || !selectedBarber || selectedServices.length === 0 || !selectedTime}
                >
