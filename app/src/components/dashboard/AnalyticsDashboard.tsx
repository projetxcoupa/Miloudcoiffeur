...
import type { Barber, Service, Appointment, Client, QueueItem } from '@/types';
...
export function AnalyticsDashboard() {
  ...
  const clientsData = useRealtime<Client>('clients', [], shopId || undefined);
  const queueItems = useRealtime<QueueItem>('queueItems', [], shopId || undefined);
  ...
  const analytics = useMemo(() => {
    ...
    // Filter completed/confirmed appointments
    const validAppointments = appointments.filter(a => a.status === 'completed' || a.status === 'confirmed');

    // Monthly Revenue & Clients
    const thisMonthApts = validAppointments.filter(a => a.startTime && new Date(a.startTime) >= startOfMonth);
    const monthlyRevenue = thisMonthApts.reduce((acc, a) => acc + (a.totalPrice || 0), 0);
    const monthlyClients = thisMonthApts.length;

    // New clients this month
    const newThisMonth = clientsData.filter(c => c.createdAt && new Date(c.createdAt) >= startOfMonth).length;
    ...
    const topServiceEntry = Object.entries(serviceCounts).sort((a, b) => b[1] - a[1])[0] || ['Aucun', 0];

    // Returning Rate
    const returningRate = clientsData.length > 0 ? Math.round((clientsData.filter(c => (c.totalVisits || 0) > 1).length / clientsData.length) * 100) : 0;

    // Charts (Last 7 days for simplicity)
    const revenueChart = [];
    const clientsChart = [];
    for (let i = 6; i >= 0; i--) {
      const date = addDays(now, -i);
      const label = format(date, 'dd/MM');
      const dayApts = validAppointments.filter(a => a.startTime && isSameDay(new Date(a.startTime), date));
      revenueChart.push({ label, value: dayApts.reduce((acc, a) => acc + (a.totalPrice || 0), 0) });
      clientsChart.push({ label, value: dayApts.length });
    }

    // Barbers Performance
    const barbersPerf = barbers.map(b => {
      const bApts = validAppointments.filter(a => a.barberId === b.id);
      return {
        barberId: b.id,
        barberName: b.name,
        revenue: bApts.reduce((acc, a) => acc + (a.totalPrice || 0), 0),
        clientsCount: bApts.length,
        averageRating: b.stats?.rating || 4.8
      };
    }).sort((a, b) => b.revenue - a.revenue);

    // Queue Metrics
    const totalApts = appointments.filter(a => ['completed', 'cancelled', 'no_show'].includes(a.status));
    const completedApts = totalApts.filter(a => a.status === 'completed');
    const noShowApts = totalApts.filter(a => a.status === 'no_show');
    const completionRate = totalApts.length > 0 ? Math.round((completedApts.length / totalApts.length) * 100) : 0;
    const noShowRate = totalApts.length > 0 ? Math.round((noShowApts.length / totalApts.length) * 100) : 0;

    const waitTimes = queueItems.map(q => q.actualWaitTime || q.estimatedWaitTime || 0).filter(t => t > 0);
    const averageWaitTime = waitTimes.length > 0 ? Math.round(waitTimes.reduce((a, b) => a + b, 0) / waitTimes.length) : 0;
    const maxWaitTime = waitTimes.length > 0 ? Math.max(...waitTimes) : 0;

    return {
      revenue: { monthly: monthlyRevenue, trend: 15 },
      clients: { monthly: monthlyClients, newThisMonth, returningRate },
      services: { topService: topServiceEntry[0], topServiceCount: topServiceEntry[1], averageServiceValue: monthlyClients ? Math.round(monthlyRevenue / monthlyClients) : 0 },
      revenueChart: revenueChart.length ? revenueChart : [{ label: 'Aujourd\'hui', value: 0 }],
      clientsChart: clientsChart.length ? clientsChart : [{ label: 'Aujourd\'hui', value: 0 }],
      barbers: barbersPerf,
      queue: { averageWaitTime, maxWaitTime, completionRate, noShowRate }
    };
  }, [appointments, clientsData, barbers, queueItems]);
  ...
  // Service distribution data mapped from real-time services source
  const serviceData = services.map((service, index) => ({
    name: service.name,
    value: (index * 15 + 50) % 100 + 50, // mock count until backend calculations exist
                ...
                {analytics.barbers.map((barber, index) => (
                  <div key={barber.barberId} className="flex items-center gap-4">
                    <Avatar className="w-10 h-10">
                      <AvatarImage src={barbers.find(b => b.id === barber.barberId)?.avatar || barbers[index]?.avatar || ''} />
