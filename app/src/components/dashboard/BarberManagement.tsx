...
export function BarberManagement() {
  ...
  const realtimeBarbers = useRealtime<Barber>('barbers', [], shopId || undefined);
  ...
  const handleStatusChange = async (barberId: string, newStatus: Barber['status']) => {
    try {
  ...
  const totalRevenue = barbers.reduce((acc, b) => acc + (b.stats?.totalRevenue || 0), 0);
  const totalClients = barbers.reduce((acc, b) => acc + (b.stats?.totalClients || 0), 0);
  ...
  const handleAddBarber = async () => {
    ...
    if (!shopId) {
      toast.error('Boutique non identifiée (shopId manquant)');
      return;
    }
    ...
    const addedBarber = {
      shopId,
      name: newBarber.name,
      avatar: `https://i.pravatar.cc/150?u=${encodeURIComponent(newBarber.name)}`,
      status: 'active',
      commissionRate: Number(newBarber.commission) || 0,
    ...
    try {
      const { data, error } = await supabase.from('barbers').insert([addedBarber]).select();
      if (error) {
          ...
          <CardContent className="p-4">
            ...
            <div className="flex items-center gap-2">
              <p className="text-2xl font-bold text-cyber-orange">
                {barbers.length > 0
                  ? (barbers.reduce((acc, b) => acc + (b.stats?.rating || 0), 0) / barbers.length).toFixed(1)
                  : '0.0'}
              </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {barbers.map((rawBarber) => {
          const barber = {
            ...
            specialties: rawBarber.specialties || [],
            schedule: rawBarber.schedule || {
              monday: { start: '09:00', end: '19:00', isWorking: true },
              tuesday: { start: '09:00', end: '19:00', isWorking: true },
              wednesday: { start: '09:00', end: '19:00', isWorking: true },
              thursday: { start: '09:00', end: '19:00', isWorking: true },
            ...
            commissionRate: rawBarber.commissionRate || 0,
          };
          return (
