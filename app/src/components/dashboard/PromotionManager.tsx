...
import type { Promotion, Client } from '@/types';
...
export function PromotionManager() {
  ...
  const realtimePromotions = useRealtime<Promotion>('promotions', [], shopId || undefined);
  ...
  const handleCreatePromotion = async () => {
    ...
    try {
      const { error } = await supabase.from('promotions').insert([{
        ...
        startDate: new Date(formData.startDate || Date.now()).toISOString(),
        endDate: new Date(formData.endDate || Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString(),
        isActive: true,
        usageCount: 0,
      }]);
  ...
  const handleSendCampaign = (type: 'sms' | 'push' | 'email') => {
    toast.success(`Campagne ${type.toUpperCase()} envoyée à ${clients.length} clients`);
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Card className="glass-card border-0">
              <CardContent className="p-6">
                ...
                <div className="flex items-center justify-between">
                  <span className="text-sm text-white/40">{clients.length} contacts</span>
                  <Button
                    variant="outline"
            ...
            <Card className="glass-card border-0">
              <CardContent className="p-6">
                ...
                <div className="flex items-center justify-between">
                  <span className="text-sm text-white/40">{clients.length} abonnés</span>
                  <Button
                    variant="outline"
              ...
              <CardContent className="p-6">
                ...
                <div className="flex items-center justify-between">
                  <span className="text-sm text-white/40">
                    {clients.filter(c => c.email).length} emails
                  </span>
                  <Button
                    variant="outline"
