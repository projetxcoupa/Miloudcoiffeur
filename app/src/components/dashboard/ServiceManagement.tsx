...
export function ServiceManagement() {
  ...
  const realtimeServices = useRealtime<Service>('services', [], shopId || undefined);
  ...
  const handleToggleActive = async (serviceId: string) => {
    ...
    try {
      ...
      toast.success(
        service.isActive ? `${service.name} désactivé` : `${service.name} activé`
      );
    } catch (e: any) {
  ...
  const handleSubmit = async () => {
    ...
    if (!shopId) {
      toast.error('Boutique non identifiée');
      return;
    }
