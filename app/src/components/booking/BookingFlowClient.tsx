...
const WheelDateTimePicker = dynamic(
    () => import('@/components/ui/WheelDateTimePicker').then(mod => mod.WheelDateTimePicker),
    { ssr: false, loading: () => <div className="h-40 flex items-center justify-center text-white/30">Chargement...</div> }
);
...
export function BookingFlowClient() {
    ...
    const queue = useRealtime<QueueItem>('queueItems', initialQueue, shopId || undefined);
    const dbServices = useRealtime<any>('services', [], shopId || undefined);
    const dbBarbers = useRealtime<any>('barbers', [], shopId || undefined);
    const [isLoading, setIsLoading] = useState(true);

    // Use DB data directly — no mock fallback
    const services = dbServices;
    const barbers = dbBarbers;

    // Track when first data load completes
    useEffect(() => {
        if (dbBarbers.length > 0 || dbServices.length > 0) {
            setIsLoading(false);
        }
        // Also set loading false after a timeout to handle empty DB
        const timer = setTimeout(() => setIsLoading(false), 3000);
        return () => clearTimeout(timer);
    }, [dbBarbers, dbServices]);
    ...
    const currentServices = services.filter(s => bookingData.selectedServices.includes(s.id));
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="flex justify-between text-sm">
                                    ...
                                    <span className="text-white font-bold">
                                        {bookingData.selectedBarber === 'any' ? 'Premier libre' : barbers.find(b => b.id === bookingData.selectedBarber)?.name}
                                    </span>
                                </div>
                                    ...
                                    try {
                                        ...
                                        if (permission === 'granted') {
                                            ...
                                            const sub = await registration.pushManager.subscribe({
                                                ...
                                                applicationServerKey: process.env.NEXT_PUBLIC_VAPID_PUBLIC_KEY || 'BOUPoYu0zPsfKnD6gM26KB8fJg1-uMoZOQnhD-Z1DW78aKOg2SY8_OrsusZWWk36DZHRyD0SowGfSYE-7PuDDkk'
                                            });
